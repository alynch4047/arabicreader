...
from printer import (initialiseRenderer, finaliseRenderer,
                     printAtPSArabic, printAtPS, printComment)
from utils import decode, getStringWidth
from sqlite import connect
from postscript import psOut
...
def printUsingFunc(func):
    global xpos, ypos
    global printBuffer

    printBuffer = []
    
    initialiseRenderer(False)

    topOfPage(False)
    
    func()
    
    lastPageNo = bufferToPrinter(False)

    finaliseRenderer(False)
...
def printEntry(QT, entryData, x, y, maxX):
    ...
    logging.debug('printentry %s', entryData)
    arabicWord1 = entryData[1]
    arabicWord2 = entryData[2]
    logging.debug( 'print ar %s', arabicWord1)
    meaning = entryData[3]
    logging.debug( 'meaning %s', meaning)
    width1 = printAtArabicBuffer(x + maxX, y, arabicWord1, QT);
    width2 = printAtArabicBuffer(x + maxX - width1 - 10, y, arabicWord2, QT);
